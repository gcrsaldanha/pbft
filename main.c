...
int prepared(request req, int sequence_number){
    ...
    for(i = 0; i<p-1; i++) received_prepares[i] = -1;
    ...
    for(i = 1; i<p; i++){
        if(i != my_rank){
            ...
            received_prepares[i-1] = (
                prep.process_id == status.MPI_SOURCE 
                && prep.view == view
    ...
    for(i = 0; i<p-1; i++){
        if(received_prepares[i] == 1) counter++;
    }
...
void primary(){
    ...
    //Predicado prepared()
    if(!prepared(req, sequence_number)) return;
...
void replica(){
    ...
    if(p_prep.process_id != status.MPI_SOURCE) return;
    if(p_prep.sequence_number != sequence_number) return;
    if(p_prep.view != view) return;
    
    request req;
    MPI_Recv(&req, 1, mpi_request, MPI_ANY_SOURCE, sequence_number, MPI_COMM_WORLD, &status);
    if(p_prep.process_id != status.MPI_SOURCE) return;
    if(req.request_type != p_prep.request_type) return;
